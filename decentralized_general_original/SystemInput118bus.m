...
% Hourly load over the overal considered horizon
PforecastTotal = HourlyLoad(:,2);
Diff_Load=HourlyLoad(:,3);
% Distribute the hourly load among the loads
LoadDistribution = LoadDist(:,2);
DTotal = zeros(Nbus,OveralHorizon);
for h = 1:OveralHorizon                                                           % Load distribution on the buses
    for j = 1:size(LoadBusIndicator,1)
        DTotal(LoadBusIndicator(j),h) = DTotal(LoadBusIndicator(j),h) + LoadDistribution(j)*PforecastTotal(h)/sum(LoadDistribution);
    end
end